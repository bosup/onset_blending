...
def get_nc_coord(ds, coord_type="lat"):
    """
    Robust coordinate getter for lat or lon from a netCDF4 Dataset.
    Tries common name variants; errors if nothing found.
    """
    import netCDF4 as nc4
    variants = (
        ["LATITUDE", "latitude", "lat", "LAT"]
        if coord_type == "lat"
        else ["LONGITUDE", "longitude", "lon", "LON", "long", "LONG"]
    )
    for v in variants:
        if v in ds.variables:
            return np.array(ds[v][:]).flatten()
    for v in variants:
        if v in ds.dimensions:
            dim = ds.dimensions[v]
            if v in ds.variables:
                return np.array(ds[v][:]).flatten()
    raise ValueError(f"Could not find {coord_type} variable/dimension in NetCDF file")
...
def nc_time_to_dates(time_num, time_units):
    ...
    time_num : array-like
        Numeric time values.
    time_units : str
        CF-compliant units string, e.g. "days since 1900-01-01".
...
def add_id_from_latlon(df, lat_col="lat", lon_col="lon", id_col="id"):
    """Add id column as 'lat_lon' if not already present."""
    df = df.copy()
    if id_col not in df.columns:
        if lat_col not in df.columns or lon_col not in df.columns:
            raise ValueError(f"Cannot create id: missing columns {lat_col} and/or {lon_col}")
#        df[id_col] = df[lat_col].astype(str) + "_" + df[lon_col].astype(str)
#        df[id_col] = (df[lat_col].round(0).astype(int).astype(str) + "_" +
#              df[lon_col].round(0).astype(int).astype(str))
        df[id_col] = (df[lat_col].map(lambda v: f"{v:.2f}") + "_" +
              df[lon_col].map(lambda v: f"{v:.2f}"))
    return df


def prep_thresholds_id(thr_df):
    """Normalize thresholds DataFrame to be keyed by id."""
    if thr_df is None:
        return None
    thr_df = thr_df.copy()
    thr_df.columns = thr_df.columns.str.lower()
    if "id" not in thr_df.columns:
        if "lat" not in thr_df.columns or "lon" not in thr_df.columns:
            raise ValueError("Thresholds table must have either id or (lat, lon).")
...
# ---------------------------------------------------------------------------
# Cell transform (optional regridding)
# ---------------------------------------------------------------------------
...
def transform_forecast_wide(df, weights_df):
    ...
    meta_cols = [c for c in df.columns if c not in day_cols + ["id", "lat", "lon", "latitude", "longitude"]]
...
def transform_groundtruth_long(df, weights_df, value_col):
    ...
    meta_cols = [c for c in df.columns if c not in ["id", value_col, "lat", "lon", "latitude", "longitude"]]
...
def nc_read_forecast_wide(nc_path, var_name, dim_rename_map, spec,
    ...
    """
    Read a forecast NetCDF into a wide-by-lead-day DataFrame.
    ...
    try:
        ...
        for d in dim_names:
            dnl = d.lower()
            if dnl in ("lat", "latitude"):
                dim_vals[d] = get_nc_coord(ds, "lat")
            elif dnl in ("lon", "longitude", "long"):
                dim_vals[d] = get_nc_coord(ds, "lon")
            elif dnl == "time":
        ...
        try:
            ...
            raise ValueError(f"Could not identify day dimension '{day_dim}' (after rename). Available: {renamed_names}")
...
def nc_read_groundtruth_long(nc_path, var_name, dim_rename_map, add_year=True):
    """
    Read a ground-truth NetCDF into a long (tidy) DataFrame.
...
def calc_onsets_rowwise(df, day_cols, day_ints, win):
    """
    Compute per-row onset indices under three restriction rules:
      raw, clim_mok_date (after June 2), mok (after MOK date).

    Returns dict: onset_raw, onset_clim_mok_date, onset_mok (each list of int|None).
    """
    ...
    #need_clim = (june2 - t0).astype("timedelta64[D]").astype(int)
...
#    print("t0 shape:", t0.shape, "dtype:", t0.dtype)
#    print("t0 sample:", t0[:5])
#    
#    print("th shape:", th.shape)
#    print("th sample:", th[:5])
#    
#    print("yr shape:", yr.shape)
#    print("yr sample:", yr[:5])
#    
#    print("june2 sample:", june2[:5])
#    print("need_clim sample:", need_clim[:5])
#    
#    print("mok_dates shape:", mok_dates.shape)
#    print("mok_dates sample:", mok_dates[:5])
#    
#    print("start_mok shape:", start_mok.shape)
#    print("start_mok sample:", start_mok[:5])
#
#    print("X shape:", X.shape)
#    print("X dtype:", X.dtype)
#
#    print("NaN count in X:", np.isnan(X).sum())
#    print("X sample (first 5 rows):\n", X[:5])
#
#    print("df shape:", df.shape)
#
#    print("day_cols:", day_cols)
#    print("Missing day_cols in df:", [c for c in day_cols if c not in df.columns])
#    
#    print("Selected df[day_cols] shape:", df[day_cols].shape)
#    print("Selected df[day_cols] head:\n", df[day_cols].head())
...
#	        print("s sample:", s[:10])
#	
#	        print("wsum_all sample:", wsum_all[:10] if len(wsum_all) > 0 else wsum_all)
#	
#	        print("sum10 sample:", sum10[:10] if len(sum10) > 0 else sum10)
#	        print("bad10 sample:", bad10[:10] if len(bad10) > 0 else bad10)
#	
#	        print("pre_bad sample:", pre_bad[:10])
#	        print("last10start:", last10start)
#	
#	        print("threshold th[i]:", th[i])
#	        print("need_clim[i]:", need_clim[i])
#	
#	        print("t0[i]:", t0[i])
#	        print("mok_dates[i]:", mok_dates[i])
#	
#	    onset_raw_val = find_onset_precomp(s, win, th[i], wsum_all, pre_bad, last10start, start_day=0)
#	    onset_raw.append(onset_raw_val)
#	
#	    onset_clim_val = find_onset_precomp(
#	        s, win, th[i], wsum_all, pre_bad, last10start,
#	        start_day=int(need_clim[i])
#	    )
#	    onset_clim_mok_date.append(onset_clim_val)
#	
#	    mk = mok_dates[i]
#	    if pd.isnull(mk):
#	        sd_mok = 0
#	    else:
#	        sd_mok = int((mk - t0[i]).astype("timedelta64[D]").astype(int))
#	
#	    onset_mok_val = find_onset_precomp(
#	        s, win, th[i], wsum_all, pre_bad, last10start,
#	        start_day=sd_mok
#	    )
#	    onset_mok.append(onset_mok_val)
#	
#	    if i < 3:
#	        print("sd_mok:", sd_mok)
#	        print("onset_raw:", onset_raw_val)
#	        print("onset_clim:", onset_clim_val)
#	        print("onset_mok:", onset_mok_val)
    ...
    for i in range(n):
        ...
        onset_raw.append(find_onset_precomp(s, win, th[i], wsum_all, pre_bad, last10start, start_day=0))
        ...
        #else:
        #    sd_mok = int((mk - t0[i]).astype("timedelta64[D]").astype(int))
        #onset_mok.append(find_onset_precomp(s, win, th[i], wsum_all, pre_bad, last10start, start_day=sd_mok))
...
def process_rainfall_forecast_id(df, spec, mok_dt=None, thr_dt=None):
    ...
    #max_number = spec.get("filter", {}).get("max_number")
...
#    print(df.iloc[:5, :])
    ...
    # Compute per-row onsets
    on = calc_onsets_rowwise(df, keep_days, keep_ints, win)
    #print("\non[onset_raw] = ", on["onset_raw"])
#    print("\non[onse_raw] not None values \n", [val for val in on["onset_raw"] if val is not None])
    #print("on[onset_clim_mok_date]", on["onset_clim_mok_date"])
    #print("on[onset_mok] = ", on["onset_mok"])
    ...
    # Stats (mean/sd/frac) aggregated over ensemble members
    ...
    # Probs aggregated over ensemble members
...
def process_ground_truth_rainfall_id(df, spec, mok_dt=None, thr_dt=None, value_col=None):
    ...
    for (cell_id, yr), g in df.groupby(["id", "year"]):
        ...
        if np.all(np.isnan(series)) or len(series) < win or np.isnan(th):
            ...
            mr_idx = find_onset_precomp(series, win, th, wsum_all, pre_bad, last10start, start_day=start_pos, reject_if_short_followup=True)# - 1)
...
#            print("\n result[wide] = \n",result["wide"].iloc[:5, :5])
#            print("result[wide] = ",result["wide"].iloc[-5:, :5])
